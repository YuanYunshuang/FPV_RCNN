...
def boxes2d_x1y1x2y2_to_xywh(boxes):
    """
    Args:
        boxes: (N, 4) or (N, C, 4)
                encoding=[x1, y1, x2, y2], C=num classes
    ...
    if dim == 2:
        x = (boxes[:, 0] + boxes[:, 2]) / 2
        y = (boxes[:, 1] + boxes[:, 3]) / 2
        w = torch.abs(boxes[:, 0] - boxes[:, 2])
        h = torch.abs(boxes[:, 1] - boxes[:, 1])
        ...
        w = torch.abs(boxes[:, :, 0] - boxes[:, :, 2])
        h = torch.abs(boxes[:, :, 1] - boxes[:, :, 1])
        return torch.stack((x, y, w, h), dim=2)
    else:
        raise ValueError("boxes dimension should be 2 or 3, but {} is given.".format(len(dim)))


def boxes2d_xywh_to_x1y1x2y2(boxes):
    """
    Args:
        boxes: (N, 4) or (N, C, 4)
                encoding=[x, y, w, h], C=num classes
    ...
    if dim == 2:
        ...
        raise ValueError("boxes dimension should be 2 or 3, but {:d} is given.".format(dim))


def clip_boxes_to_image(boxes, size, encode_type='corners'):
    # type: (Tensor, Tuple[int, int], Optional[str])
    """
    ...
    Arguments:
        ...
        encode_type (str): 'corners' or 'centroids'

    Returns:
        clipped_boxes (Tensor[N, 4])
    """
    dim = boxes.dim()
    if encode_type=='corners':
        boxes_x = boxes[..., 0::2]
        boxes_y = boxes[..., 1::2]
        height, width = size
    ...
    elif encode_type=='centroids':
        boxes = boxes2d_xywh_to_x1y1x2y2(boxes)
        clipped_boxes = clip_boxes_to_image(boxes, size, 'corners')
    else:
        raise ValueError("Encode_type should be either 'corners' or 'centroids', "
                         "but {:s} is given".format(encode_type))

    return clipped_boxes.reshape(boxes.shape)


def box3d_to_2d_bev_pixel(boxes, point_cloud_range, resolution):
    """
    ...
    centers_x = np.floor((x - point_cloud_range[0]) / resolution).astype(np.int32)
    centers_y = np.floor((y - point_cloud_range[1]) / resolution).astype(np.int32)

    new_dx = np.floor(dx / resolution).astype(np.int32)
    new_dy = np.floor(dy / resolution).astype(np.int32)

    return np.stack((centers_x, centers_y, new_dx, new_dy, theta), axis=1).astype(np.float32)


def clip_rotated_box_to_image(boxes, image_shape):
    """
    remove boxes that are outside the image
    :param boxes: (N, 5) [x, y, dx, dy, heading]
    :param image_shape: (tuple)
    :return: boxes that are inside the image
    """
    x, y, dx, dy, theta = torch.split(boxes, 1, dim=1)
    x1 = x - dx / 2
    y1 = y - dy / 2
    x2 = x + dx / 2
    y2 = y + dy / 2
    corners_x = torch.cat([x1, x1, x2, x2], dim=1)
    corners_y = torch.cat([y1, y2, y2, y1], dim=1)
    new_x = (corners_x - x) * torch.cos(theta) + (corners_y - y) * (-torch.sin(theta)) + x
    new_y = (corners_x - x) * torch.sin(theta) + (corners_y - y) * (torch.cos(theta)) + y
    num_inside_corners = ((new_x > 0).to(torch.int) * (new_y > 0).to(torch.int) \
                  * (new_x < image_shape[0]).to(torch.int) * (new_y < image_shape[1]).to(torch.int)).sum(dim=1)
    inside_corners_inds = num_inside_corners > 3

    return boxes[inside_corners_inds]
...
def remove_small_boxes(boxes, min_size):
    ...
    Arguments:
        boxes (Tensor[N, 4]): boxes in (x, y, dy, dy) format
