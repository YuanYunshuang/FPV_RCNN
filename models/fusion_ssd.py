...
class FusionSSD(nn.Module):
    ...
    def __init__(self, mcfg, cfg, dcfg):
        ...
        self.matcher = MatcherV1(mcfg.MATCHER, dcfg.pc_range, has_noise=dcfg.add_gps_noise,
    ...
    def post_processing_final(self, batch_dict, test_cfg):
        ...
        coop_boxes_corrected = [batch_dict["coop_boxes"][0]]
...
class FUDET(nn.Module):
    ...
    def fusion(self, data_dict):
        ...
        err_Ts_src = data_dict.get('err_tf_est_local', None)
        ...
        if err_Ts_src is not None:
            ...
            if coop_node_mask.sum() > 0:
                ...
                shifted_cpm_features_coop = self._affine_transform2(features_coop[coop_node_mask], -angles, -shifts_coop_to_ego)
                shifted_cpm_weights_coop = self._affine_transform2(weights_coop[coop_node_mask], -angles, -shifts_coop_to_ego)
                # fusion
            ...
            shifted_cpm_features_coop = self._affine_transform2(features_coop, -angles, -shifts_coop_to_ego)
            shifted_cpm_weights_coop = self._affine_transform2(weights_coop, -angles, -shifts_coop_to_ego)
            # shifted_cpm_features_coop = self._shift2d(features_coop, shifts_coop_to_ego.int())
